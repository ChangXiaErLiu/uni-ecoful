	...
	<view class="facility-tab">
		<view class="section-card">
			<view class="section-header">
				...
				<text class="record-count">共 {{ filteredFacilityList.length }} 条记录</text>
			</view>
			...
			<view class="section-body">
				...
				<view class="search-action-bar">
					<view class="search-container">
						<uni-easyinput v-model="facilitySearchKeyword" placeholder="搜索设施名称、处理工艺或备注..."
							prefixIcon="search" :clearable="true" @input="onFacilitySearchInput"
							class="search-input" />
						<text v-if="facilitySearchKeyword" class="search-result">
							找到 {{ filteredFacilityList.length }} 条结果
						</text>
					</view>

					<view class="action-buttons">
						<button class="btn btn-add" @tap="handleAddFacility">
							<uni-icons type="plus" size="18" color="#ffffff" />
							<text>新增</text>
						</button>
						<button class="btn btn-refresh" @tap="handleRefreshFacility" :disabled="loadingFacility">
							<uni-icons :type="loadingFacility ? 'spinner-cycle' : 'refresh'" size="18"
								:color="loadingFacility ? '#94a3b8' : '#ffffff'" />
							<text>{{ loadingFacility ? '加载' : '刷新' }}</text>
						</button>
					</view>
				</view>

				<!-- 加载状态 -->
				<view v-if="loadingFacility" class="loading-container">
					<uni-icons type="spinner-cycle" size="48" color="#166534" class="loading-spinner" />
					<text class="loading-text">正在加载设施数据...</text>
				</view>

				<!-- 错误状态 -->
				<view v-else-if="fetchFacilityError" class="error-container">
					<uni-icons type="close-circle" size="48" color="#dc2626" />
					<text class="error-message">加载失败: {{ fetchFacilityError }}</text>
					<button class="btn btn-retry" @tap="handleFetchFacility">
						<text>重新加载</text>
					</button>
				</view>

				<!-- 设施表格 -->
				<view v-else-if="filteredFacilityList.length" class="facility-table">
					<!-- 表格头部 - 在PC端显示 -->
					<view class="table-header">
						<view class="header-cell header-index">序号</view>
						<view class="header-cell header-name">设施名称</view>
						<view class="header-cell header-quantity">处理工艺</view>
					...
					<view class="table-body">
						<view v-for="(item, index) in paginatedFacilityList" :key="item.id" class="table-row"
							...
							<view class="body-cell cell-name">
								<view class="mobile-label">设施名称</view>
							...
							<view class="body-cell cell-quantity">
								<view class="mobile-label">处理工艺</view>
								<uni-easyinput v-model="item.quantity" placeholder="处理工艺" :clearable="true"
									class="cell-input" />
							</view>
							...
							<view class="body-cell cell-action">
								...
								<button class="btn-delete" @tap="() => handleRemoveFacility(index)">
				...
				<view v-else class="empty-container">
					<uni-icons :type="facilitySearchKeyword ? 'search' : 'flag'" size="64" color="#e2e8f0" />
					<text class="empty-title">
						{{ facilitySearchKeyword ? '未找到匹配的设施' : '暂无设施信息' }}
					</text>
					<text class="empty-tip">
						{{ facilitySearchKeyword ? '试试其他关键词或清空搜索' : '点击"新增设施"按钮开始添加' }}
					</text>
					<button v-if="!facilitySearchKeyword" class="btn btn-add-primary" @tap="handleAddFacility">
