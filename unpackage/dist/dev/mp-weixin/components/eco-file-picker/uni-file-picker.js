...
const components_ecoFilePicker_chooseAndUploadFile = require("./choose-and-upload-file.js");
const components_ecoFilePicker_utils = require("./utils.js");
...
const _sfc_main = {
  ...
  created() {
    if (!(common_vendor.tr.config && common_vendor.tr.config.provider)) {
      this.noSpace = true;
      common_vendor.tr.chooseAndUploadFile = components_ecoFilePicker_chooseAndUploadFile.chooseAndUploadFile;
    }
  ...
  methods: {
     ...
     */
    chooseFiles() {
      const _extname = components_ecoFilePicker_utils.get_extname(this.fileExtname);
      common_vendor.tr.chooseAndUploadFile({
        ...
        common_vendor.index.__f__("log", "at components/eco-file-picker/uni-file-picker.vue:369", "选择失败", err);
      });
    },
    /**
     * 选择文件回调
    ...
    async chooseFileCallback(res) {
      ...
      let {
        filePaths,
        files
      } = components_ecoFilePicker_utils.get_files_and_is_max(res, _extname);
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        let filedata = await components_ecoFilePicker_utils.get_file_data(files[i], this.fileMediatype);
        filedata.progress = 0;
    ...
    uploadFiles(files) {
      files = [].concat(files);
      return components_ecoFilePicker_chooseAndUploadFile.uploadCloudFiles.call(this, files, 5, (res) => {
        this.setProgress(res, res.index, true);
      }).then((result) => {
        this.setSuccessAndError(result);
        return result;
      }).catch((err) => {
        common_vendor.index.__f__("log", "at components/eco-file-picker/uni-file-picker.vue:457", err);
      });
    },
    /**
     * 成功或失败
...
//# sourceMappingURL=../../../.sourcemap/mp-weixin/components/eco-file-picker/uni-file-picker.js.map
