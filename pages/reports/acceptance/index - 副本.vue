...
<script setup>
	...
	async function handleFileSelect(e) {
		...
		uni.showLoading({
			title: `上传中 (0/${supportedFiles.length})...`,
			mask: true
		})

		// 统计对象属性名与 showUploadResult 匹配
		const stats = {
			successCount: 0,
			failCount: 0,
			supported: 0, // 可解析的文件数
			nonSupported: 0, // 不支持解析的文件数（图片等）
		...
		for (let i = 0; i < supportedFiles.length; i++) {
			...
			try {
				uni.showLoading({
					title: `上传中 (${i + 1}/${supportedFiles.length})...`,
					mask: true
				})
		...
		if (stats.supported > 0) {
			...
			await handleAutoIndexBuild(stats.supported) //这里传递的是 supported 数量
		}
	}
